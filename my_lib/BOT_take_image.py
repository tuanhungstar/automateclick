...
import urllib.error    # For exception handling
...
# --- Github Image Downloader Dialog ---
class GithubFilesDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Get Image from Github")
    ...
    def fetch_files(self):
        try:
            with urllib.request.urlopen("https://api.github.com/repos/tuanhungstar/automateclick/contents/Click_image") as response:
                if response.status == 200:
                    data = json.loads(response.read().decode())
                    self.github_files = [file['name'] for file in data if file['type'] == 'file']
                    self.filter_files()
                else:
                    QMessageBox.critical(self, "Error", f"Failed to fetch files from Github. Status: {response.status}")
        except (urllib.error.URLError, json.JSONDecodeError) as e:
            QMessageBox.critical(self, "Error", f"Failed to fetch files from Github: {e}")


    def filter_files(self):
        search_text = self.search_bar.text().lower()
        self.file_list_widget.clear()
        for file_name in self.github_files:
            if search_text in file_name.lower():
                self.file_list_widget.addItem(file_name)

    def get_selected_files(self):
        return [item.text() for item in self.file_list_widget.selectedItems()]

# ... (The rest of your code for Old_utility remains the same) ...
class Old_utility:
    def __int__(self):
...
class Ui_MainWindow_Base(object):
    def setupUi(self, MainWindow):
        ...
        self.tabWidget.addTab(self.screenshot_tab, "Screenshot")
        
        self.KPI_botcomment_txt = QtWidgets.QListWidget(parent=self.screenshot_tab)
        self.KPI_botcomment_txt.setGeometry(QtCore.QRect(130, 40, 231, 121))
        self.KPI_botcomment_txt.setObjectName("KPI_botcomment_txt")
        self.exit_BOT_butt = QtWidgets.QPushButton(parent=self.screenshot_tab)
        self.exit_BOT_butt.setGeometry(QtCore.QRect(10, 130, 51, 28))
        self.exit_BOT_butt.setObjectName("exit_BOT_butt")
        self.Start_BOT_butt = QtWidgets.QPushButton(parent=self.screenshot_tab)
        self.Start_BOT_butt.setGeometry(QtCore.QRect(10, 40, 111, 28))
        self.Start_BOT_butt.setObjectName("Start_BOT_butt")
        self.label = QtWidgets.QLabel(parent=self.screenshot_tab)
        self.label.setGeometry(QtCore.QRect(50, 10, 71, 16))
        self.label.setObjectName("label")
        self.conf_list_cob_img = QtWidgets.QComboBox(parent=self.screenshot_tab)
        ...
        self.click_img = QtWidgets.QLabel(parent=self.screenshot_tab)
        ...
        self.label_3 = QtWidgets.QLabel(parent=self.screenshot_tab)
        self.label_3.setGeometry(QtCore.QRect(20, 180, 101, 16))
        self.label_3.setObjectName("label_3")
        self.delete_img_butt = QtWidgets.QPushButton(parent=self.screenshot_tab)
        self.delete_img_butt.setGeometry(QtCore.QRect(10, 100, 111, 28))
        self.delete_img_butt.setObjectName("delete_img_butt")
        self.label_test_result = QtWidgets.QLabel(parent=self.screenshot_tab)
        ...
        self.conf_list_cob_window = QtWidgets.QComboBox(parent=self.screenshot_tab)
        ...
        # --- Github Tab ---
        self.github_tab = QtWidgets.QWidget()
        self.github_tab.setObjectName("github_tab")
        self.tabWidget.addTab(self.github_tab, "Github")

        self.get_github_files_butt = QtWidgets.QPushButton(parent=self.github_tab)
        self.get_github_files_butt.setGeometry(QtCore.QRect(10, 10, 121, 28))
        self.get_github_files_butt.setObjectName("get_github_files_butt")
        
        self.github_filter_txt = QtWidgets.QLineEdit(parent=self.github_tab)
        self.github_filter_txt.setGeometry(QtCore.QRect(140, 10, 181, 28))
        self.github_filter_txt.setObjectName("github_filter_txt")
        
        self.github_files_list = QtWidgets.QListWidget(parent=self.github_tab)
        self.github_files_list.setGeometry(QtCore.QRect(10, 40, 311, 121))
        self.github_files_list.setObjectName("github_files_list")
        self.github_files_list.setSelectionMode(QtWidgets.QAbstractItemView.SelectionMode.ExtendedSelection)

        self.github_image_view = QtWidgets.QLabel(parent=self.github_tab)
        self.github_image_view.setGeometry(QtCore.QRect(330, 10, 321, 151))
        self.github_image_view.setFrameShape(QtWidgets.QFrame.Shape.Box)
        self.github_image_view.setText("")
        self.github_image_view.setObjectName("github_image_view")

        self.download_github_files_butt = QtWidgets.QPushButton(parent=self.github_tab)
        self.download_github_files_butt.setGeometry(QtCore.QRect(10, 170, 151, 28))
        self.download_github_files_butt.setObjectName("download_github_files_butt")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.get_github_files_butt.setText(_translate("MainWindow", "Get List of Files"))
        self.download_github_files_butt.setText(_translate("MainWindow", "Download Selected Files"))
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow_Base):
    ...
    def __init__(self, statup_image_param: str):
        ...
        self.get_github_files_butt.clicked.connect(self.fetch_github_files)
        self.download_github_files_butt.clicked.connect(self.download_selected_github_files)
        self.github_filter_txt.textChanged.connect(self.filter_github_files)
        self.github_files_list.itemClicked.connect(self.preview_github_image)
    ...
    def fetch_github_files(self):
        try:
            with urllib.request.urlopen("https://api.github.com/repos/tuanhungstar/automateclick/contents/Click_image") as response:
                if response.status == 200:
                    data = json.loads(response.read().decode())
                    self.github_files_list.clear()
                    for file in data:
                        if file['type'] == 'file':
                            self.github_files_list.addItem(file['name'])
                else:
                    self.show_info_messagebox(f"Failed to fetch files from Github. Status: {response.status}")
        except (urllib.error.URLError, json.JSONDecodeError) as e:
            self.show_info_messagebox(f"Failed to fetch files from Github: {e}")

    def filter_github_files(self):
        filter_text = self.github_filter_txt.text().lower()
        for i in range(self.github_files_list.count()):
            item = self.github_files_list.item(i)
            item.setHidden(filter_text not in item.text().lower())
            
    def preview_github_image(self, item):
        file_name = item.text()
        try:
            url = f"https://raw.githubusercontent.com/tuanhungstar/automateclick/main/Click_image/{file_name}"
            with urllib.request.urlopen(url) as response:
                if response.status == 200:
                    image_data = response.read()
                    pixmap = QtGui.QPixmap()
                    pixmap.loadFromData(image_data)
                    self.github_image_view.setPixmap(pixmap.scaled(self.github_image_view.size(), QtCore.Qt.AspectRatioMode.KeepAspectRatio))
                else:
                    self.show_info_messagebox(f"Failed to preview image. Status: {response.status}")
        except urllib.error.URLError as e:
            self.show_info_messagebox(f"Failed to preview image: {e}")
            
    def download_selected_github_files(self):
        selected_items = self.github_files_list.selectedItems()
        if not selected_items:
            self.show_info_messagebox("No files selected for download.")
            return

        for item in selected_items:
            file_name = item.text()
            self.download_file_from_github(file_name)
        
        self.update_file_list()

    def download_file_from_github(self, file_name):
        try:
            url = f"https://raw.githubusercontent.com/tuanhungstar/automateclick/main/Click_image/{file_name}"
            with urllib.request.urlopen(url) as response:
                if response.status == 200:
                    with open(os.path.join('Click_image', file_name), 'wb') as f:
                        f.write(response.read())
                    self.show_info_messagebox(f"Successfully downloaded {file_name}")
                else:
                    self.show_info_messagebox(f"Failed to download {file_name}. Status: {response.status}")
        except urllib.error.URLError as e:
            self.show_info_messagebox(f"Failed to download {file_name}: {e}")
